...
import com.hx.mqtt.domain.req.mqtt.TaskPauseReq;
...
public class TaskPauseHandler implements MqttTopicHandler {

    private final RcsApiService rcsApiService;

    @Override
    public TopicEnum getTopicEnum() {
        return TopicEnum.TASK_PAUSE;
    }

    @Override
    public MqttResp<?> handle(MqttContext context, String payload) {
        TaskPauseReq req = TaskPauseReq.fromJson(payload);
        String msg = rcsApiService.taskPause(req.getAmrId());
        if (msg == null) {
